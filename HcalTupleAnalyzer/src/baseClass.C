...
TProfile* baseClass::makeTProfile(const char * name, int nbins, float xmin, float xmax ){
  TProfile* hist = new TProfile(name, name, nbins, xmin, xmax );
  m_objects.push_back(hist);
  return hist;
}
TProfile* baseClass::makeTProfile(const char * name, const char * title, int nbins, float xmin, float xmax ){
...
void baseClass::printTriggers(){
  ...
  std::map<std::string, bool>::iterator j     = triggerDecisionMap_.begin();
  std::cout << "Triggers include:" << std::endl;
  std::cout << "\t" << "Prescale" << "\t" << "Decision" << "\t\"" << "Name" << "\"\t" << std::endl;
  for (; i != i_end; ++i, ++j) std::cout << "\t" << i -> second << "\t" << j -> second << "\t\"" << i -> first << "\"" << std::endl;
}
