...
void HBStudy_plotByQIE::Loop()
{
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
    ...
    std::vector <int> check_ieta;
    std::vector <int> check_iphi;
    ...
    std::ofstream out ("Hcal_Events_366323.txt", std::ios::app);
    if (out.is_open()){
	...
	//if ((fg0 + fg1 + fg2 + fg3 ) > 0) out << jentry << " " << TPieta << " " << TPiphi << " " << HcalTriggerPrimitiveFineGrain0->at(ch).at(TS) << HcalTriggerPrimitiveFineGrain1->at(ch).at(TS) << HcalTriggerPrimitiveFineGrain2->at(ch).at(TS)<< HcalTriggerPrimitiveFineGrain3->at(ch).at(TS) << std::endl;
	
	//if ((fg0 + fg1 + fg2 + fg3 ) > 0 ) {
	//if (TPieta == 7 && TPiphi == 41 && HcalTriggerPrimitiveCompressedEt->at(ch).at(TS) == 18) {
	  //if (num == 1) continue;
	  //for (int TSl = 0; TSl < 4; TSl++) {
	  //int TSl = 2;
	//std::cout << "HC Event " << jentry << ": HCAL TP tower with set FG bits 0, 1, 2, 3  ("<< fg0 << fg1 << fg2 << fg3<< "), with TP ieta, iphi, TS = " << TPieta << ", " << TPiphi <<  ", " << TS << " Energy " << HcalTriggerPrimitiveCompressedEt->at(ch).at(TS) << std::endl;
	//std::cout << "HC Event " << jentry << ": HCAL TP tower with set FG bits 0, 1, 2, 3  ("<< HcalTriggerPrimitiveFineGrain0->at(ch).at(TSl) << HcalTriggerPrimitiveFineGrain1->at(ch).at(TSl) << HcalTriggerPrimitiveFineGrain2->at(ch).at(TSl) << HcalTriggerPrimitiveFineGrain3->at(ch).at(TSl)<< "), with TP ieta, iphi, TS = " << TPieta << ", " << TPiphi <<  ", " << TSl << " Energy " << HcalTriggerPrimitiveCompressedEt->at(ch).at(TSl) << std::endl;
	//out << "HC Event " << jentry << ": HCAL TP tower with set FG bits 0, 1, 2, 3 ("<< HcalTriggerPrimitiveFineGrain0->at(ch).at(TSl) << HcalTriggerPrimitiveFineGrain1->at(ch).at(TSl) << HcalTriggerPrimitiveFineGrain2->at(ch).at(TSl) << HcalTriggerPrimitiveFineGrain3->at(ch).at(TSl) << "), with TP ieta, iphi, TS =  " << TPieta << ", " << TPiphi <<  ", " << TSl << " Energy " << HcalTriggerPrimitiveCompressedEt->at(ch).at(TSl) << std::endl;
	//num = 1;
	 //}
	  
	  
	  //check_ieta.push_back (TPieta);
	  //check_iphi.push_back (TPiphi);
	//}
    ...
    for (int ch = 0; ch < QIE11DigiIEta->size(); ++ch) {
      ...
      //std::cout << "HC Event " << jentry << " at ieta, iphi, depth " << ch_ieta  << ", " << ch_iphi << ", " << ch_depth << std::endl;
	...
	if (HB_cell_valid[ch_ieta].find(ch_depth) == HB_cell_valid[ch_ieta].end()) HB_cell_valid[ch_ieta][ch_depth] = new TH1F(Form("HB_cell_valid_ieta%d_depth%d",ch_ieta,ch_depth),"Valid cell;QIE Relative Shift (ns);Fraction of Events",11,-2,9);	
	 ...
	 //int maxTS = 0;
	 //for ( int i = 0; i < check_ieta.size(); i++){
	   //for (int TS = 0; TS < 8; TS++) {
	    
	    //int TS = 2;

	     //if (ch_ieta == check_ieta.at(i) && ch_iphi == check_iphi.at(i) && QIE11DigiTDC->at(ch).at(TS) < 4){
	     
	        //std::cout << "HC Event " << jentry << ": with QIE11 digi TDC = " << QIE11DigiTDC->at(ch).at(TS) << " at ieta, iphi, depth, TS = " << ch_ieta << ", " << ch_iphi << ", " << ch_depth << ", " << TS <<" Energy " << QIE11DigiADC->at(ch).at(TS)<< std::endl;
	        //out << "HC Event " << jentry << ": with QIE11 digi TDC = " << QIE11DigiTDC->at(ch).at(TS) << " at ieta, iphi, depth, TS = " << ch_ieta << ", " << ch_iphi << ", " << ch_depth << ", " << TS << " Energy " << QIE11DigiADC->at(ch).at(TS)<< std::endl;
	    //}
	   //}
	...
	if ((QIE11DigiADC->at(ch).at(3) > ADC_xml_decoded[ch_ieta - 1][ch_iphi-1][ch_depth - 1] && ch_ieta > 0) || (QIE11DigiADC->at(ch).at(3) > ADC_xml_decoded[ch_ieta * -1 + 16 - 1][ch_iphi-1][ch_depth - 1] && ch_ieta < 0)) {
	  if (QIE11DigiTDC->at(ch).at(3) < 3) std::cout << "HC Event " << jentry << ": with QIE11 digi TDC = " << QIE11DigiTDC->at(ch).at(3) << " at ieta, iphi, depth = " << ch_ieta << ", " << ch_iphi << ", " << ch_depth << "Energy " << QIE11DigiADC->at(ch).at(3) << std::endl;
